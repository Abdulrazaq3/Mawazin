...
const QuickActions: React.FC<{ setCurrentPage: (page: Page) => void }> = ({ setCurrentPage }) => (
  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 sm:rtl:space-x-reverse">
    ...
    <button onClick={() => setCurrentPage('المعاملات')} className="flex items-center justify-center bg-white text-slate-700 font-semibold py-2 px-5 rounded-lg border border-slate-300 hover:bg-slate-50 transition btn-press">
      <PlusIcon />
...
const UpcomingRemindersTable: React.FC<{ reminders: Reminder[], handleMarkReminderAsPaid: (id: number) => void, isLoading: boolean }> = ({ reminders, handleMarkReminderAsPaid, isLoading }) => (
    <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-slate-200/80">
      <h3 className="text-lg font-bold text-slate-800 mb-4">التذكيرات القادمة</h3>
        ...
        <table className="w-full text-right min-w-[640px]">
          <thead>
            <tr className="border-b border-slate-200">
              ...
              <th className="p-4 text-sm font-semibold text-slate-500 text-right uppercase tracking-wider">الاستحقاق</th>
          ...
          <tbody className="divide-y divide-slate-100">
             {isLoading ? <SkeletonLoader rows={3} cols={5} /> : reminders.length > 0 ? reminders.map((reminder, index) => (
              <tr key={reminder.id} className="odd:bg-white even:bg-slate-50/50 hover:bg-teal-50/50 transition-colors duration-200 opacity-0 animate-stagger-in" style={{ animationDelay: `${index * 50}ms` }}>
                <td className="p-4 whitespace-nowrap font-semibold text-slate-700">{reminder.tenantName}</td>
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          <h2 className="text-3xl sm:text-4xl font-bold text-slate-900">أهلاً بعودتك!</h2>
          <p className="text-slate-600 mt-1">إليك نظرة عامة على أعمالك اليوم.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {isLoading ? (
          Array.from({ length: 4 }).map((_, i) => (
             <div key={i} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200/80 h-[130px] animate-pulse">
                <div className="h-8 w-8 bg-slate-200 rounded-md mb-4"></div>
